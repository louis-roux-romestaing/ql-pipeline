...
def get_recent_batches(df_yc, min_year=2022, keep_last_n=6):
    VALID_PREFIXES = {"W", "S"}  # only Winter and Summer batches
    batches = set()
    for batch in df_yc["batch"].dropna().unique():
        m = re.search(r'(\d{2})$', str(batch))
        prefix = str(batch)[0].upper() if batch else ""
        if m and int("20" + m.group(1)) >= min_year and prefix in VALID_PREFIXES:
            batches.add(batch)
...
def main():
    ...
    results = []
    with ThreadPoolExecutor(max_workers=1) as ex:  # keep low to avoid GitHub rate limits
    ...
    # data/latest/ — always current, overwritten each run
    os.makedirs("data/latest", exist_ok=True)
    df_final.to_csv("data/latest/full.csv", index=False)
    df_bc.to_csv("data/latest/series_bc.csv", index=False)
    df_a.to_csv("data/latest/series_a.csv", index=False)
    print(f"✓ Latest → data/latest/")

    # data/history/YYYY-MM-DD/ — dated snapshot, never overwritten
    hist = f"data/history/{run_date}"
