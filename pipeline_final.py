...
def get_recent_batches(df_yc, min_year=2022, keep_last_n=4):
    """Keep only the most recent N batches — oldest auto-drops when new batch arrives"""
    batches = set()
    for batch in df_yc["batch"].dropna().unique():
        match = re.search(r'(\d{2})$', str(batch))
        if match and int("20" + match.group(1)) >= min_year:
            batches.add(batch)
    # Sort and keep only last N (e.g. W23, S23, W24, S24 — drops W22 when W25 arrives)
    sorted_batches = sorted(batches)
    return set(sorted_batches[-keep_last_n:])


RESCORE_TOP_N       = 50   # rescore top N every week to keep rankings fresh
RESCORE_RECENCY     = 90   # also rescore companies added in last N days
KEEP_LAST_N_BATCHES = 6    # rolling window — oldest batch drops when new one arrives
...
def get_new_companies(df_yc):
    ...
    try:
        ...
        # ── Drop companies from batches no longer in rolling window ──
        if "batch" in df_existing.columns:
            ...
            if dropped > 0:
                print(f"Dropped  : {dropped} companies from old batches")

        existing_ids = set(df_existing["id"].astype(str))

        # 1. Brand new companies never seen before
        df_new = df_filtered[~df_filtered["id"].astype(str).isin(existing_ids)].copy()

        # 2. Top 50 by last known score — rescore weekly to keep rankings fresh
        if "final_score" in df_existing.columns:
            top_ids = set(df_existing.nlargest(RESCORE_TOP_N, "final_score")["id"].astype(str))
        else:
            top_ids = set()

        # 3. Recently added companies (last 90 days) — new GitHub data may have come in
        if "last_scored" in df_existing.columns:
            cutoff = (NOW - timedelta(days=RESCORE_RECENCY)).isoformat()
            recent_ids = set(df_existing[df_existing["last_scored"] >= cutoff]["id"].astype(str))
        else:
            recent_ids = set()
        ...
        # Remove companies being rescored so they get fresh rows
        df_existing_clean = df_existing[~df_existing["id"].astype(str).isin(rescore_ids)]

        print(f"Existing : {len(df_existing)} companies")
        print(f"New      : {len(df_new)} (never seen)")
        print(f"Rescore  : {len(df_rescore)} (top {RESCORE_TOP_N} + recent {RESCORE_RECENCY}d)")
        print(f"To process this run: {len(df_to_process)}")
